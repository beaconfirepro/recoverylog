...
code and the patient's date of birth. `src/lib/joinCode.js` checks both, and the
date-of-birth field on the join screens is still to be added, so these answers
are correct for the decision and a step ahead of the screens.
